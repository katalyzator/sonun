...
def project(request, slug):
    ...
    more_products = Product.objects.filter(category__in=Category.objects.filter(name=project.category.name)).exclude(
        slug=slug)
    testis = []
...
def service(request, slug):
    ...
    more_services = Service.objects.filter(category__in=Category.objects.filter(id=service.category.id)).order_by(
        '-publish').exclude(slug=slug)
    ...
    if request.is_ajax():
        ...
        if get_email == '':
            ...
            mail.send_messages(subject=_('Thank you for choosing SONUN'),
                               ...
                               context={'subscriber': get_email, 'contacts': Contact.objects.first(),
                                        'unsubscribe': get_email},
                               to_emails=[get_email])
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail.send_messages(subject=_('New request from: {client}'),
                               ...
                               context={'fullname': str(new_letter.fullname), 'telephone': new_letter.telephone.strip(),
                                        'email': new_letter.email, 'contacts': Contact.objects.first()},
